...
const templateWithJQuery = () => {
  ...
  for (let i = 0; i < starWarsPeople.length; i++){
    ...
    $clonedSection.find('h2').text(starWarsPeople[i].name);
    $clonedSection.find('h3').text(starWarsPeople[i].height);
    $clonedSection.find('p').text(starWarsPeople[i].eye_color);
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++){
    ...
    let indexOfFirstSpace = ingredient.indexOf(searchSpace);
    let secondSpace = ingredient.indexOf(searchSpace, (indexOfFirstSpace + 1));
