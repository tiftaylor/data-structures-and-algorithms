...
const createServer = () => {
  ...
  app.get('/hello', (req, res) => res.send('greeting'));
  app.get('/aboutme', (req, res) => res.send('this is a bio that i am sending to the front end. so it has two sentences.'));
  app.get('/favoritefoods', (req, res) => res.send(['a', 'b', 'c']));
...
const count = (target, input) => {
  return input.reduce((accum, innerArr) => {
    const foundNum = innerArr.filter(val => target === val);
    return accum + foundNum.length;
  } , 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(arr => arr.filter( divByFive => {
    if (typeof divByFive === 'number') {
      return divByFive % 5 === 0;
    } else {
      return false;
    }
