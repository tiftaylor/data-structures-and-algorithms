...
let biggerThanLuke = (arr) => {
  ...
  if (luke) {
    const lukeMass = arr[0].mass;
    const bigBois = arr.filter(obj => parseInt(obj.mass) > parseInt(lukeMass));
    const bigBoiNames = bigBois.map(obj => obj.name);

    return bigBoiNames.join(' - ');
  } 
...
const sortBy = (property, arr) => {
  ...
  const stuff = arr.sort((a, b) => (a[property] > b[property]) ? 1 : -1);
  return stuff;
